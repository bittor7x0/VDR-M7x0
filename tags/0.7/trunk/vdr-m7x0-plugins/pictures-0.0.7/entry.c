...
void cPictureEntry::Load(void) const
{
  if (isDirectory && !entries) {
     ...
     if (d.Ok()) {
        struct dirent *e;
        while ((e = d.Next()) != NULL) {
//			isyslog("cEntry::load dir name:%s (%s)", e->d_name,strrchr(e->d_name,'.'));
              if (strcmp(e->d_name, ".") && strcmp(e->d_name, "..")) {
                 struct stat ds;
                 if (stat(AddDirectory(Directory, e->d_name), &ds) == 0) {
//					 isyslog("Load %s, isdir:%d",e->d_name,S_ISDIR(ds.st_mode));
					if (S_ISDIR(ds.st_mode) || (strrchr(e->d_name,'.')!=NULL && !strcmp(strrchr(e->d_name,'.'), ".iFrame"))){
						if (!entries)
