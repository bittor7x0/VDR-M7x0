...
** 930507 JLI   [1.60 mwTest & CHECK_MEMORY()]
...
**      Detect: Always on CHECK_MEMORY(), but can be detected in several places.
...
**  Check:
**      An error was detected during a CHECK_MEMORY() operation.
**      The associated pointer is displayed along with
**      the file and line where the CHECK_MEMORY() was executed.
**      Followed immediately by a normal error message.
**      Detect: When you CHECK_MEMORY()
...
**  - mwAutoCheck() performs a CHECK_MEMORY() operation whenever a MemWatch function
...
**  - CHECK_MEMORY() does a complete memory integrity test. Slow!
...
/* Renamed from CHECK to CHECK_MEMORY because VDR used CHECK for 'ioctl()' calls */
#define CHECK_MEMORY()  mwTest(__FILE__,__LINE__,MW_TEST_ALL)
