...
cOsdServerMenu::cPrivateMenu::~cPrivateMenu() {
    ...
    if (ParentMenu() && !ParentMenu()->Dirty) {
        if (ParentMenu()->IsActive) ParentMenu()->TriggerEvent(cEvent::BlurEvent);
        ParentMenu()->TriggerEvent(cEvent::CloseEvent);
    }
...
eOSState cOsdServerMenu::cPrivateMenu::ProcessKey(eKeys Key) {
    ...
    if (ParentMenu()->SubMenu) {
        ...
        if (!ParentMenu()->IsActive) ParentMenu()->TriggerEvent(cEvent::FocusEvent);
...
void cOsdServerMenu::Detach() {
    ...
    if (IsActive) TriggerEvent(cEvent::BlurEvent);
    IsActive = false;
}
...
void cOsdServerMenu::GetCurrent(int &cur, const char *&curs) {
    cur = current;
...
cOsdServerMenuEditStrItem::cPrivateMenuEditStrItem::cPrivateMenuEditStrItem(cOsdServerMenuEditStrItem *parent)
    ...
    ParentMenuEditStrItem()->value = (char*)malloc(ParentMenuEditStrItem()->length);
