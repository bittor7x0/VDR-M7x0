...
{
  // Remove ".iFrame" extension
  isyslog("Para:%s",s);
  char *p = strrchr(s,'.');
  isyslog("Ext:%s Num:%d",p,p-s);
  s[p-s]='\0';
} */
...
{
  bool Force = false;
  cString Path = RemoveExtension(pictureEntry->Path());
  ...
  int h = 2 * Font->Height();
  if (!osd) {
     osd=cOsdProvider::NewOsd(Setup.OSDLeft,Setup.OSDTop+ Setup.OSDHeight - h);
...
eOSState cPictureControl::ProcessKey(eKeys Key)
{
  switch (Key) {
    ...
    case kGreen:  NextDirectory(-1);
                  slideShow = false;
                  break;
    case kFastFwd:
    case kFastFwd|k_Repeat:
    case kYellow: NextDirectory(+1);
                  slideShow = false;
                  break;
    case kBlue:
    case kStop:   return osEnd;
    case kBack:   if (osd && !alwaysDisplayCaption){
					DELETENULL(osd);
				  }
				  slideShow = false;
