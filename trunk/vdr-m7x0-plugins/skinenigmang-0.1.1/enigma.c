...
cSkinEnigmaDisplayChannel::cSkinEnigmaDisplayChannel(bool WithInfo)
{
  ...
  if ((SingleArea[0].bpp < 8 || EnigmaConfig.singleArea8Bpp) && osd->CanHandleAreas(SingleArea, sizeof(SingleArea) / sizeof(tArea)) == oeOk) {
    ...
    if (SingleArea[0].bpp >=8 /* && Setup.sAntiAlias */)
      nBPP = 8;
  } else {
...
void cSkinEnigmaDisplayChannel::UpdateSignal() {
  ...
  int xSignalBarRight = xSignalBarLeft + MIN_CI_SIGNALBAR;

  str = str * MIN_CI_SIGNALBAR / 0xAA;//0xFFFF;     // SIGNAL => Scale 0..255
  snr = snr * MIN_CI_SIGNALBAR / 0x19;//0xFFFF;     // BER    => Scale 0..25
  ...
  if (str != nStrBarWidth || snr != nSnrBarWidth) {
    ...
    if (str <= 37/*0.5 * MIN_CI_SIGNALBAR*/) // low signal : RED
      signalFgColor = Theme.Color(clrSignalLowFg);
    else if (str <= 44/*0.6 * MIN_CI_SIGNALBAR*/) // medium signal : ORANGE
    ...
    if (snr <= 37/*0.5 * MIN_CI_SIGNALBAR*/) // low signal : RED
      signalFgColor = Theme.Color(clrSignalLowFg);
    else if (snr <= 44/*0.6 * MIN_CI_SIGNALBAR*/) // medium signal : ORANGE
      signalFgColor = Theme.Color(clrSignalMediumFg);
