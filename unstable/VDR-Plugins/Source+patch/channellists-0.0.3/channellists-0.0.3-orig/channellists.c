...
cMenuSetupChannellists::cMenuSetupChannellists(void) {
  ...
  Add(new cMenuEditBoolItem(tr("Button Red"),           &newRedButtonSwitch,   tr("Rename"), tr("Skript")));
  Add(new cMenuEditBoolItem(tr("Button Green"),         &newGreenButtonSwitch, tr("Copy"),   tr("Skript")));
  }
...
protected:
  virtual void Action(void) {
     if (access(UpdateScript, 01) == 0) {
        esyslog("Channellists: executing %s", UpdateScript);
        system(UpdateScript);
        }
     else
