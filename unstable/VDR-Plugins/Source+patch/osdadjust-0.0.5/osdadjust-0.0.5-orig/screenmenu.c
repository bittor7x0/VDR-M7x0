...
//  dsyslog("osdadjust: max OSD size is %dx%d @ %s", Width, Height, VideoAspectString[Aspect]);
  width = Width;
...
void cScreenMenu::DrawGrid(int tmpFgColor, int tmpBgColor)
{
  ...
  if (config.AspectRatio == 3) x = 40;    // 1:1
  ...
  if (config.AspectRatio == 3) offset = 0;    // 1:1
