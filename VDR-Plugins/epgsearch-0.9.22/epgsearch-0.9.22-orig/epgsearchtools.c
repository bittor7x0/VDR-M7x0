...
bool CopyFile(const char* file_path_from,  const char* file_path_to)
{
   ...
   char buf[MAXPARSEBUFFER+1];   /* input buffer.          */

   /* open the source and the target files. */
   f_from = fopen(file_path_from, "r");
   if (!f_from) {
      LogFile.eSysLog("Cannot open source file: %s", file_path_from);
      return false;
   }
   f_to = fopen(file_path_to, "w+");
   if (!f_to) {
      LogFile.eSysLog("Cannot open target file: %s", file_path_to);
      return false;
   }

   /* copy source file to target file, line by line. */
   while (fgets(buf, MAXPARSEBUFFER+1, f_from)) {
      if (fputs(buf, f_to) == EOF) {  /* error writing data */
         LogFile.eSysLog("Error writing to target file: %s", file_path_to);
         return false;
      }
   }
   if (!feof(f_from)) { /* fgets failed _not_ due to encountering EOF */
      LogFile.eSysLog("Error reading from source file: %s", file_path_from);
      return false;
   }

   /* close source and target file streams. */
   if (fclose(f_from) == EOF) {
      LogFile.eSysLog("Error when closing source file: %s", file_path_from);
      return false;
   }
   if (fclose(f_to) == EOF) {
      LogFile.eSysLog("Error when closing target file: %s", file_path_to);
      return false;
   }
   return true;
}
   ...
   while(tmp)
   {
      ...
      if (!delim || 
         ...
         if (strlen(line) > 0)
         {
            if (rawDescr)
            {
               char* tmp = strdup(rawDescr);
               free(rawDescr); 
               rawDescr = NULL;
               asprintf(&rawDescr, "%s%s", tmp, line);
               free(tmp);
            }
            else
               rawDescr = strdup(line);
         }
...
{
   if (!CopyFile(from.c_str(), to.c_str()))
   {
      LogFile.Log(1, "error moving %s to %s", from.c_str(), to.c_str());
      return false;
   }
   bool success = (remove(from.c_str()) == 0);
   if (!success)
      LogFile.Log(1, "error removing %s", from.c_str());
   return success;
}
