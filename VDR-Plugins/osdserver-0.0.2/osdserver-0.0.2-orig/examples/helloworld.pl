...
sub EditUserComment() {
    SendCmd "submenu=New Menu 'Edit User comment'" or return $false;
    SendCmd "submenu.SetColumns 15" or return $false;
    SendCmd "submenu.EnableEvent keyOk close" or return $false;

    SendCmd "submenu.AddNew OsdItem -unselectable '--- Who do you want to comment? --------------------'" or return $false;
    SendCmd "who=submenu.AddNew EditListItem Who OSDServer VDR Linux Windows Klaus Bill -SelectName '$who'" or return $false;
    SendCmd "who.SetCurrent" or return $false;
    
    SendCmd "submenu.AddNew OsdItem -unselectable '--- And how do you like it? --------------------'" or return $false;
    SendCmd "what=submenu.AddNew EditStrItem 'It is' '$what'" or return $false;
    
    SendCmd "menu.addsubmenu submenu" or return $false;
    SendCmd "submenu.show" or return $false;
    
    while (1) {
        SendCmd "submenu.SleepEvent" or return $false;

        if (IsEvent("submenu","keyOk")) {
            ...
            SendCmd "submenu.SendState osBack" or return $false;
            SendCmd "delete submenu" or return $false;    
            return $true;
        }
        if (IsEvent("submenu","close")) {
            SendCmd "delete submenu" or return $false;    
            return $true;
        }
    }    
