...
function EditUserComment() {
    SendCmd "submenu=New Menu 'Edit User comment'" || return $false
    SendCmd "submenu.SetColumns 15" || return $false
    SendCmd "submenu.EnableEvent keyOk close" || return $false

    SendCmd "submenu.AddNew OsdItem -unselectable '--- Who do you want to comment? --------------------'" || return $false
    SendCmd "who=submenu.AddNew EditListItem Who OSDServer VDR Linux Windows Klaus Bill -SelectName '$who'" || return $false
    SendCmd "who.SetCurrent" || return $false
    
    SendCmd "submenu.AddNew OsdItem -unselectable '--- And how do you like it? --------------------'" || return $false
    SendCmd "what=submenu.AddNew EditStrItem 'It is' '$what'" || return $false
    
    SendCmd "menu.addsubmenu submenu" || return $false
    SendCmd "submenu.show" || return $false
    
    while true; do
        SendCmd "submenu.SleepEvent" || return $false

        if IsEvent submenu keyOk ; then
            ...
            SendCmd "submenu.SendState osBack" || return $false
            SendCmd "delete submenu" || return $false    
            return $true
        fi
        if IsEvent submenu close ; then
            SendCmd "delete submenu" || return $false    
            return $true
        fi
    done   
