...
#define msecs_to_jiffies(msecs) (msecs/10)
#define input_sync(x...)
#define init_input_dev(dev) memset(dev, 0, sizeof(struct input_dev))
...
static
void ws_timer_callback (unsigned long data)
{
	schedule_task((struct tq_struct *) data);
}

static
void INIT_WORK (struct work_struct *ws, void (*func) (void *data), void *data)
{
	INIT_TQUEUE(&ws->tqueue, func, data);
	init_timer(&ws->timer);
	ws->timer.function = ws_timer_callback;
	ws->timer.data = (unsigned long) &ws->tqueue;
}

static
void schedule_delayed_work (struct work_struct *ws, int delay_jiffies)
{
	ws->timer.expires = jiffies + delay_jiffies;
	add_timer(&ws->timer);
}

#define cancel_delayed_work(ws) del_timer_sync(&(ws)->timer)
...
static
void* cinergyt2_probe (struct usb_device *udev, unsigned intf,
...
static void cinergyt2_disconnect (struct usb_device *udev, void *data)
{
	struct cinergyt2 *cinergyt2 = (struct cinergyt2 *) data;
